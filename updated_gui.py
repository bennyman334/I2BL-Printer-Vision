...
class DashboardApp(ctk.CTk):
    def __init__(self):
        ...
        self.displacements = []
        self.syringeOffsets = np.array([1.7, -22, -40]) #x, y, z offsets to bring syringe to top right hole
        ...
        zr_buttons = ctk.CTkFrame(self.xyz_controls, fg_color="transparent")
        zr_buttons.pack(pady=(4, 10))
        ...
        ctk.CTkButton(
            ...
            width = 80, 
            height = 30,
            command = lambda: self.zr_move("A", float(self.z_step_var.get()))
        ).pack(pady=4)
    ...
    def zr_move(self, direction, amount):
        send_gcode("G91")
        send_gcode(f'G1 {direction}{amount} F200')
        #send_gcode("G90")
    ...
    def moveSyringe(self):
        ...
        send_gcode("G91")
        send_gcode("G1 X{} F200".format(self.syringeOffsets[0]))
        send_gcode("G1 Y{} F200".format(self.syringeOffsets[1]))
        send_gcode("G1 Z{} F200".format(self.syringeOffsets[2]))
