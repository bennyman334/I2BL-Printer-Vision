...
def detect_green_region(frame: np.ndarray, debug_draw: np.ndarray = None) -> tuple[np.ndarray, tuple[float, float], tuple] | None:
    ...
    if not show_capture:
        ...
        if detected is not None:
            ...
            if steady_detection_count >= STEADY_THRESHOLD and not auto_capture_done:
                ...
                #Calibrate only once using dot detection 
                PIXELS_PER_MM = calibration(ordered, 23.5)

                #Detect and measure green tape 
                result = detect_green_region(display_frame, display_frame)
                if result is not None:
                    green_box, (w_px, h_px), rect = result 
                    _, _, angle = rect 

                    #Make sure width is horizontal, length is vertical
                    if w_px > h_px:
                        width_px = w_px 
                        length_px = h_px 
                    else:
                    ...
                    length_mm = round(length_px / PIXELS_PER_MM, 1) 
                    size_array = [round(length_mm, 1), round(width_mm, 1)]

                    print(f'[SIZE] Tape dimensions (px): [{round(length_px, 1)}, {round(width_px, 1)}]')
                    print(f'[SIZE] Tape dimensions (mm): [{round(length_mm, 1)}, {round(width_mm, 1)}]')
